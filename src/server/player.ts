import { Connection, Frame, Logger, Priority, Status } from "@sanctumterra/raknet";
import { Server } from "./server";
import { decodeLoginJWT, Emitter, PacketCompressor, PacketEncryptor } from "../libs";
import { ClientOptions, defaultClientOptions, PacketNames, Payload, ClientData, LoginData, prepareLoginData } from "../client";
import { DataPacket, getPacketId, NetworkSettingsPacket, Packets, PacketViolationWarningPacket, PlayStatus, PlayStatusPacket, ServerToClientHandshakePacket } from "@serenityjs/protocol";
...
export class Player extends Emitter<PlayerEvents> {
    ...
    constructor(server: Server, connection: Connection) {
        ...
        this.options.compressionThreshold = this.server.options.compressionThreshold;
        this.prepare();
    }
    ...
    public processPacket(buffer: Buffer) {
        ...
        try {
            ...
            Logger.error(`Received an Error while deserializing packet ${PacketClass ?? id}`)
    ...
    public prepare() {
        ...
        this.on("LoginPacket", async (packet) => {
            ...
            const pubKeyDer = createPublicKey({ key: Buffer.from(key, "base64"), format: "der", type: "spki" });
