import { ClientCacheStatusPacket, ClientToServerHandshakePacket, ResourcePackClientResponsePacket, ResourcePackResponse, getPacketId, Packets, ResourcePacksInfoPacket, StartGamePacket, DataPacket, PlayStatus, SetLocalPlayerAsInitializedPacket, ServerboundLoadingScreenPacketPacket, ServerboundLoadingScreenType, RequestNetworkSettingsPacket, RequestChunkRadiusPacket } from "@serenityjs/protocol";
import { Profile, Emitter, createOfflineSession, authenticate, PacketCompressor, PacketEncryptor, ProtocolList, CurrentVersionConst } from "../";
import { ClientData, ClientEvents, ClientOptions, defaultClientOptions, PacketNames, WorkerClient } from "./";
import { Advertisement, Frame, Logger, Priority, Client as RaknetClient, Status } from "@sanctumterra/raknet";
...
export class Client extends Emitter<ClientEvents> {
    ...
    constructor(options: Partial<ClientOptions>) {
        ...
        this.once("session", () => { this.sessionReady = true; });
